...
class signalrClient {
  ...
  _negotiate() {
    return new Promise((resolve, reject) => {
      ...
      let negotiateRequestOptions = url.parse(
        `${this.url}/negotiate?${query}`,
        true
      );
          ...
          try {
            if (res.statusCode == 200) {
              ...
              if (!negotiateProtocol.TryWebSockets) {
                return reject({
                  code: errorCode.unsupportedWebsocket,
                  message: null,
                });
              }
  ...
  _start() {
    return new Promise((resolve, reject) => {
      ...
      startRequestOptions.headers = {
        ...this.headers,
        "User-Agent": "WebSocket++/0.5.1",
      };
...
class hub {
  ...
  on(hubName, methodName, cb) {
    let handler = this.handlers[hubName];
    if (!handler) {
      handler = this.handlers[hubName] = {};
    }
    handler[methodName] = cb;
  }
